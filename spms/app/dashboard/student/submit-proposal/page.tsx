  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const groupRes = await fetch("/api/project-groups", {
        ...
        body: JSON.stringify({
          ProjectGroupName: formData.groupName,
          ProjectTypeID: Number(formData.projectTypeId),
          GuideStaffID: Number(formData.guideStaffId),
          ProjectTitle: formData.projectTitle,
          ProjectArea: formData.projectArea,
          ProjectDescription: formData.projectDescription,
          AverageCPI:
            selectedMembers.reduce((sum, id) => {
      ...
      const projectGroupId = groupData.id
      ...
      for (const memberId of selectedMembers) {
        ...
        const memberRes = await fetch("/api/project-group-members", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ProjectGroupID: projectGroupId,
            StudentID: Number(memberId),
            IsGroupLeader: memberId === currentStudent?.id,
            StudentCGPA: student?.cgpa || 0,
            Description:
              memberId === currentStudent?.id ? "Group Leader" : "Group Member",
          }),
        })
...
//   const currentStudent = students.find((s) => s.email === user?.email) || students[0]
//   // const [myProject, setMyProject] = useState<SubmitProject[]>([]);
...
//   //   if (!formData.ProjectName || !formData.projectTitle || !formData.projectTypeId || !formData.guideStaffId) {
//   //     toast.error("Please fill all required fields")
//   //     return
//   //   }
...
//       const averageCpi =
//         members.reduce((acc, m) => acc + m.cgpa, 0) / members.length
...
//       if (!response.ok) {
//         throw new Error("Failed to submit project proposal")
//       }
...
//     } catch (error) {
//       console.error(error)
//       toast.error("Something went wrong while submitting proposal")
//     }
//   }
