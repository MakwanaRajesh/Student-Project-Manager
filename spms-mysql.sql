...
ALTER TABLE STUDENT
ADD SPI DECIMAL(3,1) NOT NULL;

ALTER TABLE Student
ADD SPI DECIMAL(3,1) DEFAULT 0.0;
update Student SET SPI where StudentName = "Rajesh Kumar" 
...
CREATE TABLE ProjectGroup (
    ...
    -- Foreign Keys
    CONSTRAINT fk_projectgroup_projecttype
        FOREIGN KEY (ProjectTypeID) 
        REFERENCES ProjectType(ProjectTypeID),

    CONSTRAINT fk_projectgroup_guide
        FOREIGN KEY (GuideStaffID) 
        REFERENCES Staff(StaffID),

    CONSTRAINT fk_projectgroup_convener
        FOREIGN KEY (ConvenerStaffID) 
        REFERENCES Staff(StaffID),

    CONSTRAINT fk_projectgroup_expert
        FOREIGN KEY (ExpertStaffID) 
        REFERENCES Staff(StaffID)
);

select * from ProjectGroup;

SELECT 1 FROM ProjectGroupMember
WHERE StudentID = ?
LIMIT 1;


INSERT INTO ProjectGroup
(ProjectGroupName, ProjectTypeID, GuideStaffID, ProjectTitle, ProjectArea, ProjectDescription, AverageCPI, ConvenerStaffID, ExpertStaffID, Description, Status)
VALUES
('SPMS Group A', 1, 1, 'Student Project Management System', 'Web Development',
 'A system to manage student academic projects', 7.25, 2, 3, 'Approved Project', 'approved');
...
select * from ProjectGroupMember;
