...
export default function StudentSubmitProposalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedMembers.length === 0) {
      toast.error("Please add at least yourself as a member")
      return
    }

    setIsLoading(true)
    try {
      const members = selectedMembers.map((studentId) => {
        const student = students.find((s) => s.id === studentId)
        return { studentID: parseInt(studentId), isGroupLeader: studentId === currentStudent?.id, studentCGPA: student?.cgpa || 0 }
      })

      await addProjectGroup({
        projectGroupName: formData.groupName,
        projectTypeID: parseInt(formData.projectTypeId),
        guideStaffID: parseInt(formData.guideStaffId),
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex justify-end gap-4">
                ...
                <Button type="submit" disabled={isLoading}>{isLoading ? "Submitting..." : "Submit Proposal"}</Button>
              </div>
            </div>
