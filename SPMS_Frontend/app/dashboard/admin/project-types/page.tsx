...
export default function AdminProjectTypesPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await addProjectType({ projectTypeName: formData.name, description: formData.description })
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      await updateProjectType(editingType.id, { projectTypeName: formData.name, description: formData.description })
  ...
  const handleDelete = async (id: string) => {
    ...
    if (projectsUsingType.length > 0) { toast.error(`Cannot delete: ${projectsUsingType.length} projects using this type`); return }
    try { await deleteProjectType(id); toast.success("Project type deleted") } catch (err: unknown) { toast.error(err instanceof Error ? err.message : "Failed") }
  }
