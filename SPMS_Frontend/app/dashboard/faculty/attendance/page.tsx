...
export default function FacultyAttendancePage() {
  const { user, projectGroups, meetings, students, staff, updateMeeting, updateAttendance } = useAppStore()
  ...
  const handleSaveAttendance = async () => {
    ...
    const attendances = group.members.map((member) => ({
      studentID: parseInt(member.studentId),
      isPresent: attendance[member.studentId] ?? false,
      attendanceRemarks: "",
    }))

    try {
      await updateAttendance(meeting.id, attendances)
